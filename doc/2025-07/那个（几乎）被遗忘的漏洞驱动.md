...
通过在 IOCTL 调用的返回缓冲区中传递当前进程的 token 地址，_SEP_TOKEN_PRIVILEGES_TOKEN  
 结构会被多次覆盖，每次递增 1。值得注意的是，uVar7  
 变量是一个由 4 字节组成的无符号整型。  
...
- 从 Windows 11 24H2 / Windows Server 2025 开始，查询 LSASS 进程还需要启用 SeDebugPrivilege  
...
在这种情况下，我们需要从偏移量 0x231 开始，因为我们需要将 PreviousMode  
 设置为 0 而不是 1 😉  
